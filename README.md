...
| 压缩包 | 给谁使用 | 作用 |
|---|---|---|
| `china-payroll-workflow-universal-v<version>.zip`（通用版） | 领导、财务人员 | 解压后双击 `打开工资表工具.html`，生成待复核工资表 |
| `china-payroll-workflow-maintenance-v<version>.zip`（维护版） | 领导电脑上的 Codex 或开发人员 | 包含源码、合成测试、规则说明和重新打包工具，用于维护或增加表单适配 |
...
- 目标月附件即使与上月数值相同也会按来源重新写入；
- 一次选择多少份目标月附件就读取多少份，同一工作表的多个变动区段分别处理；
- 有身份证的人员变动严格按身份证匹配，不在未命中时静默改用姓名；
- 劳务费与当前草案金额不同只显示财务复核提醒，临时劳务费或本月新增不阻断写入；
- 病假、事假按已确认公式形成考勤扣款；转正天数等必要资料缺失时停止；
- 绩效、提成、其他工资和扣款等一次性项目先重置，再等待目标月资料；
...
本工具不是“放入附件就自动得到正式工资表”。完整流程有两道人工审核：

1. 生成前，在页面核对附件是否已读取、月份是否正确、人员与字段是否匹配，并确认本月
   人员和工资变动；
2. 生成后，在桌面版 Excel 中完整重算，由财务逐项复核金额、公式和例外，再决定是否
   转为正式工资表。
...
2. 双击 `打开工资表工具.html`；
3. 选择上月已经确认的完整工资表；
4. 一次选择目标月份收到的全部有效附件，程序会全部读取并按内容分类；
5. 输入或核对人员、工资、考勤、绩效、提成及已验证业务 Excel/Word 中的目标月变化；
6. 如果工作簿有打开密码，按弹窗输入；每个文件可使用不同密码；
7. 逐项确认“已读取”、人员、字段、月份提醒和错误预览；
8. 生成待复核工资表，再用桌面版 Excel 完整重算并由财务复核。
